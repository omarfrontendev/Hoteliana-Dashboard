...
import { fetchAgents } from "@/app/store/features/agents/agentsThunk";
import { useAgentsColumns } from "../components/AgentsColumns";

export const useAgentssTableLogic = () => {
    const dispatch = useDispatch<AppDispatch>();

    const [statusDialog, setStatusDialog] = useState(null);

    // Select suppliers state from Redux store
    const { agents, loading, total, error } = useSelector((state: any) => state.agents);
    ...
    useEffect(() => {
        const { pageIndex, pageSize, search, isActive, isFirstActivationPending } = tableOptions;
    ...
    const columns: ColumnDef<any>[] = useAgentsColumns(setRefreshData, setStatusDialog);
